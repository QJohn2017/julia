...
.. function:: pmap([::WorkerPool], f, c...; distributed=true, batch_size=1, on_error=nothing, retry_on_error=e->true, retry_n=0, retry_max_delay=0) -> collection
   ...
   Failed computation can also be retried via ``retry_on_error``\ , ``retry_n``\ , ``retry_max_delay``\ , which are passed through to ``retry`` as arguments ``on_error``\ , ``n`` and ``max_delay`` respectively. If batching is specified, and an entire batch fails, all items in the batch are retried.

   The following are equivalent :     ``pmap(f, c; distributed=false)`` and ``asyncmap(f,c)``     ``pmap(f, c; retry_n=1)`` and ``asyncmap(retry(remote(f)),c)``     ``pmap(f, c; retry_n=1, on_error=e->e)`` and ``asyncmap(x->try retry(remote(f))(x) catch e; e end, c)``
